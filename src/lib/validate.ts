import SvelteTurnstileError from '$lib/error'
import type { TurnstileTokenValidateResponse } from '$lib'
import { CHALLENGE_URL, ERROR_MESSAGE } from '$lib/variables'


export default async (turnstileResponse: FormDataEntryValue, secret: string) => {
  if (!turnstileResponse) throw new SvelteTurnstileError(ERROR_MESSAGE)
  else {
    ...
    if (!validationResponse.success) throw new SvelteTurnstileError(ERROR_MESSAGE)
  }
}
